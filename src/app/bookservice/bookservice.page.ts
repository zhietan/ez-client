...
export class BookservicePage {
  ...
  updateMyMinHours(event : any)
  {
    ...
    if(new Date(event.detail.value) > open)
    {
  ...
  updateMyDurasi(event : any)
  {
    var today = new Date();
    var open = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 18, 0, 0);
    var selectdate = new Date(event.detail.value);
    var selected = new Date(selectdate.getFullYear(), selectdate.getMonth(), selectdate.getDate(), selectdate.getHours(), selectdate.getMinutes(), 0);
    if(selected >= open)
    {
      this.timeDisabled = true;
    }
