...
export class MarkerPage implements OnInit {
  ...
  updatePositionSocket(marker,map){

    console.log('load partner location');
    this.socket.on("get-latlong", (resp) => {
      console.log('socket get loc',resp);
      this.latitude = resp.lat;
      this.longitude = resp.long;
      console.log('upate latlong',this.latitude,this.longitude);
      
      var latlng = new google.maps.LatLng(resp.lat, resp.long);
     ...
     this.geo.watchPosition().subscribe(position => {
      if ((position as Geoposition).coords != undefined) {
        ...
        this.socket.emit('lat-long', {
          ...
          customer_id : this.user_data.customer_id,
  ...
  connectSocket(){
    ...
    this.socket.connect();
    this.socket.emit('is-online', { user: '1234' })
  ...
  loadMap(latOri, lngOri, latDest, lngDest) {
    ...
    this.interval = setInterval(() => {
      ...
      // this.watchPosision()
      this.updatePositionSocket(marker,map)
    }, 5000);
  }

  call(){
    window.open('tel:' + this.data_.phone);
  }
